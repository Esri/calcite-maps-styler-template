...
define(
     ({
        ...
        tooltips: {
            ...
            filter: "Φιλτράρισμα επιπέδων χάρτη",  // Explains purpose of type-in box affiliated with template picker
            basemap: "Αλλαγή βασικού χάρτη",  // Command button to open a dialog box for switching basemaps
        ...
        prompts: {
            ...
            mapLayers: "Επίπεδα χάρτη:",  // Appears before a list of map layers; shown when the app is not configured with the layer to use for the find command; works with the searchLayerMissing message
            layerFields: "Πεδία επιπέδου εύρεσης:"  // Appears before a list of fields in the configured map find layer; shown when the app cannot find one or more of the fields that were configured for the find command; works with the searchFieldMissing message
        },
        messages: {
            ...
            noSearchLayerConfigured: "Δεν έχει διαμορφωθεί επίπεδο εύρεσης",  // Appears before a list of map layers; shown when the app is not configured with any layers to use for the find command; works with the mapLayers prompt
            searchLayerMissing: "Αυτό το επίπεδο εύρεσης δεν βρέθηκε στο χάρτη",  // Appears before a list of map layers; shown when the app is not configured with the layer to use for the find command; works with the mapLayers prompt
            searchLayerNotSearchable: "Δεν ήταν δυνατή η εύρεση των πεδίων για το επίπεδο χάρτη.<br><br>Επαληθεύστε ότι αυτό το επίπεδο υπάρχει στη ρίζα των περιεχομένων του χάρτη. Οι ένθετες υπηρεσίες, όπως οι δυναμικές υπηρεσίες χάρτη του ArcGIS for Server, πρέπει να προστίθενται στο χάρτη ένα επίπεδο τη φορά (συμπεριλαμβανομένου του αριθμού ευρετηρίου επιπέδου) για να χρησιμοποιηθούν ως επίπεδα εύρεσης. Οι υπηρεσίες πλακιδίων δεν μπορούν να χρησιμοποιηθούν ως επίπεδα εύρεσης.",
            searchFieldMissing: "Αυτό το πεδίο δεν βρέθηκε στο επίπεδο εύρεσης του χάρτη",  // Appears before a list of fields in the configured map find layer; shown when the app cannot find one or more of the fields that were configured for the find command; works with the layerFields prompt
            allSearchFieldsMissing: "Κανένα από αυτά τα πεδία δεν βρέθηκε στο επίπεδο εύρεσης του χάρτη",  // Appears before a list of fields in the configured map find layer; shown when the app cannot find any of the fields that were configured for the find command; works with the layerFields prompt
            fieldNotFound: "Αυτό το πεδίο δεν υπάρχει σε κανένα από τα επίπεδα του χάρτη",  // Appears when a field used in the configuration was not found in any map layer
