...
public class proxy : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context) {
        ...
        try {
            log(TraceLevel.Info,requestUri);  //???
    ...
    private static void logMessageToFile(String message)
    {
        ...
        if (config.LogFile != null)
        {
            ...
            if (!log.Contains("\\") || log.Contains(".\\"))
            {
                ...
                string configDirectory = HttpContext.Current.Server.MapPath("proxy2.config"); //Cannot use System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath b/ config may be in a child directory
                string path = configDirectory.Replace("proxy2.config","");
                log = path + log;
            }
...
public class ProxyConfig
{
    ...
    public static ProxyConfig GetCurrentConfig() {
        ...
        if (config == null) {
            string fileName = HttpContext.Current.Server.MapPath("proxy2.config");
