...
9624d94519a4d5b0 2013-11-04 15:19:54 -0800
...
function(){g.reject(null)})):g.reject(null)},function(){g.reject(null)})},function(a){g.reject(a)}):g.resolve(null);b&&"object"===typeof b?(m=b,k.resolve(b)):b&&"string"===typeof b||e.app||c?(p=b&&"string"===typeof b?b:e.app?e.app:c,d.loadFromFile(p).then(function(a){h=a.ui||null;q=a.values||null;k.resolve(d)},function(a){k.reject(a)})):k.resolve(null);(new u([g,k])).then(function(a){!a[0]||!a[1]?d.ready.reject(d):(m?(d.uiSpecification=m,console.log("Using override object for UI")):h&&"string"===
typeof b?(d.uiSpecification=h,console.log("Using override file "+b+".json for UI")):n?(d.uiSpecification=n,console.log("Using appId "+e.appid+" for UI")):h&&e.app?(d.uiSpecification=h,console.log("Using app file "+e.app+".json for UI")):h&&c?(d.uiSpecification=h,console.log("Using default file "+c+".json for UI")):console.warn("No UI found"),d.values=q||{},dojo.mixin(d.values,s),dojo.mixin(d.values,r),dojo.mixin(d.values,e),d.values=d.organizeConfigValues(d.values),d.values.Shared=d.values.Shared||
{},d.values.Shared.commonConfig=l,d.ready.resolve(d))})})},parseQueryString:function(a,b){var c={};(a||"?").substr(1).replace(/([^&=]+)=?([^&]*)(?:&+|$)/g,function(a,e,f){b?c[e]=f:(c[e]=c[e]||[],c[e].push(f))});return c},parseJSON:function(a){try{return window.JSON.parse(a)}catch(b){}},organizeConfigValues:function(a){var b,c,d={};for(b in a)a.hasOwnProperty(b)&&(c=b.split("."),1===c.length?(d.Shared||(d.Shared={}),d.Shared[b]=a[b]):(d[c[0]]||(d[c[0]]={}),d[c[0]][c[1]]=unescape(a[b])));return d},
launch:function(){var a=this,b=new f;setTimeout(function(){dojo.forEach(a.uiSpecification,function(b){a.instantiateClass(b)});b.resolve()});return b},instantiateClass:function(a){var b,c=null;try{b=this.reduceToVariableName(a.classname),a.styles&&this.injectCSS(a.styles),a.config&&(a.config.i18n=this.i18n,a.config.rootId&&dojo.mixin(a.config,this.getValues(a.config.rootId)||{}),dojo.mixin(a.config,this.getValues("Shared"))),c=new (eval(b))(a.config)}catch(d){}return c},reduceToVariableName:function(a){return a.replace(/[^\w\d\x2e\x5f]/g,
...
a.sharingUrl);!a.proxyUrl&&-1!==b&&(a.proxyUrl=location.protocol+"//"+location.host+c+"/sharing/proxy");console.log("proxyUrl: "+a.proxyUrl);esri.request({url:a.sharingUrl+"/sharing/rest/portals/self",content:{f:"json"},callbackParamName:"callback"}).then(function(b){a.self=b;b.isPortal&&"single tenant"===b.portalMode&&(a.sharingUrl=b.portalHostname);x.mixin(a.helperServices,b.helperServices);esri.arcgis.utils.arcgisUrl=a.sharingUrl+"/sharing/rest/content/items";a.helperServices&&(a.helperServices.geometry&&
a.helperServices.geometry.url)&&(esri.config.defaults.geometryService=new esri.tasks.GeometryService(a.helperServices.geometry.url));d.resolve(!0)});a.proxyUrl&&(esri.config.defaults.io.proxyUrl=a.proxyUrl,esri.config.defaults.io.alwaysUseProxy=!1);return d}})});
