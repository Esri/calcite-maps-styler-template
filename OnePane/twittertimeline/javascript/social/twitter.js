...
dojo.declare("social.twitter", null, {
    ...
    updateTime: function(){
        //if only one graphic no point in slider 
    if(this.featureLayer.graphics.length > 1){
          //update the time slider to display time range for tweets
          var dateField = this.featureLayer.timeInfo.startTimeField;
          var minDate, maxDate;

          var minGraphic = this.featureLayer.graphics[this.featureLayer.graphics.length - 1];

          if (minGraphic) {
            minDate = new Date(this.featureLayer.graphics[this.featureLayer.graphics.length - 1].attributes[dateField]);
          } else {
            minDate = new Date(this.featureLayer.graphics[0].attributes[dateField]);
          }

          maxDate = new Date(this.featureLayer.graphics[0].attributes[dateField]);



          var tweetTimeExtent = new esri.TimeExtent(minDate, maxDate);


          this.timeExtent = tweetTimeExtent;
    ...
    mapResults: function (j) {
        ...
        //get the geocoded points and add to the map 
        tweets = this.getGeocodedLocations(j.statuses);
        if(tweets.length > 0){
            this.featureLayer.applyEdits(tweets, null, null);
            this.onUpdate();
        }else{
            console.log("No Geocoded Tweets");
        }
    },

    getGeocodedLocations: function(locations){
        var geocodedLocations = [];

        dojo.forEach(locations, dojo.hitch(this, function(result){

            result.smType = this.options.id;
            //update the time 
            if(result['created_at']){
                result['created_at'] = new Date(result['created_at']).getTime();
            }
            //is the tweet already displayed on the map? 
            ...
            //Does the tweet have a geocoded location? 
            var geoPoint = null;
            if(result && result.geo && result.geo.coordinates){
                var g = result.geo.coordinates;
                geoPoint = new esri.geometry.Point(parseFloat(g[1]), parseFloat(g[0]));
            }else if(result && result.location){
                //Try some different parsings for the result location 
                ...
                if(n.indexOf("iPhone:") > -1){
                    ...
                    //location cannot be interpreted by the geocoder 
                    this.stats.geoNames++;
                    return;
                }
            }else{
            ...
            //Is there a geocoded point? 
            if(geoPoint){
                //last check to make sure location was parsed correctly
                if(isNaN(geoPoint.x) && isNaN(geoPoint.y)){
                    //discard bad locations
                    this.stats.noGeo++;
                }else{
                    //display the point on the map 
                    var graphic = new esri.Graphic(geoPoint);
                    graphic.setAttributes(result);
                    geocodedLocations.push(graphic);
