define(
   ({
    ...
    user: {
      ...
      invalidString: "請輸入有效值。",
      invalidSmallNumber: "請輸入有效${openStrong}整數${closeStrong}值，範圍在 -32768 到 32767 之間。",
      ...
      requiredFields: "存在一些錯誤。請在下方進行更正。",
      requiredField: "(必填)",
      error: "錯誤",
      ...
      invalidLayerMessage: "表單圖層不存在。請配置應用程式並設置該圖層。",
      ...
      exceededFileCountError: "超出允許的最大附件數",
    ...
    builder: {
      ...
      titleText: "構建器",
      descriptionText: "GeoForm 是一種可配置範本，可用於編輯${link1}圖徵服務${closeLink}的表單式數據。使用者可透過該應用程式使用表單輸入數據(而非透過地圖的快顯)，同時使用 ${link2}Web 地圖${closeLink}的強大功能和可編輯圖徵服務。請使用以下步驟自訂並部署 GeoForm。",
      btnPreviousText: "上一步",
      btnNextText: "下一步",
      ...
      shareBuilderTextMessage: "可透過與其他成員共用開始收集資訊",
      shareModalFormText: "表單連結",
      shareBuilderSuccess: "已更新並發佈 GeoForm!",
      geoformTitleText: "地理表單",
      layerTabText: "這是要建立 GeoForm 的圖層。該圖層必須是圖徵服務，且為您的觀眾啟用了編輯共用權限。",
      detailsTabText: "使用下方的“表單詳細資訊”方塊自訂標題、新增自訂標誌並為 GeoForm 觀眾提供簡短描述。您可以在描述中新增其他資源的連結和聯繫資訊，甚至可以將您的觀眾指向包含所有透過 GeoForm 所收集數據的 Web 製圖應用程式。",
      fieldsTabText: "您可以在此處選擇對 GeoForm 觀眾可見的欄位，編輯觀眾可見的標記並新增簡短描述以說明輸入數據。",
      styleTabText: "根據首選項使用下方的主題設置 GeoForm 樣式。",
      publishTabText: "如果已完成對 GeoForm 的自訂，請儲存應用程式並將其與觀眾共用。您可隨時返回此構建器並根據意見反應繼續進行自訂。",
      ...
      shareGeoformText: "共用 Geoform",
      shareDescription: "透過共用面板，觀眾可以在提交後與其他協作者輕鬆共用 GeoForm - 可隨時停用此功能。",
      ...
      showHideLayerHelpText: "可配置 GeoForm 以顯示/隱藏圖層。此選項僅適用於單個圖層配置。",
      labelHelpMessage: "標籤",
      placeHolderHintMessage: "提示文字",
      placeHolderHelpMessage: "說明文字",
      selectTextOptionValue: "篩選選擇",
      disableLogo: "停用標誌",
      disableLogoDescription: "可配置 GeoForm 以在表單標題中顯示/隱藏標誌",
      locateOnLoadText: "在載入中定位",
      locateOnLoadDescription: "可配置 GeoForm 以使用頁面載入中的目前位置",
      selectLayerFieldTabText: "選擇圖層",
      allLayerSelectOptionText: "全部",
      disableViewer: "停用檢視器",
      disableViewerDescription: "可配置 GeoForm 以停用/啟用檢視器",
    ...
    viewer: {
      ...
      mapTabTooltip: "地圖",
      appLoadingFailedMessage: "檢視器模式無法使用",
      btnDescendingText: "降序",
      btnAscendingText: "遞增",
      geometryUnavailableErrorMessage: "無法找到圖徵幾何",
      infoPopupOffErrorMessage: "資訊快顯已關閉",
      btnLoadMoreText: "載入更多",
      unavailableTitleText: "無標題",
      unavailableConfigMessage: "無法載入配置"
    }
  })
);