		...
		viewer: {
			...
			helpPopup: {
				...
				tab1: {
					...
					div5: "我们非常期待您的参与! 如果您有任何疑问、想要请求新要素或者发现漏洞，请访问<a href='http://links.esri.com/storymaps/forum' target='_blank'>故事地图用户论坛</a>。"
				},
				tab2: {
					title: "数据",
				...
				tab4: {
					...
					div4a: "为浏览写入适当的子标题",
					div4b: "花点时间为浏览写入引人注目的子标题。子标题是吸引人们关注您的浏览并简要介绍浏览内容的良好途径。它也是告知用户该浏览所定位的州或国家的好方法。例如，不要假设用户知道浏览中城市或城镇的位置。还可使用 HTML 标签来设置说明文字的格式，例如包括链接。但说明文字不要过长。在较小的浏览器窗口或 iPad 中，子标题可能并不完全适合，需要进行裁剪。如果您未想出合适的子标题，则将其留空。",
					div5: "支持图层",
					...
					div12: "避免使用狭窄的 iFrame 宽度，这可能导致地图浏览切换至其相应的小屏幕触控式布局。要尽可能地方便使用，在嵌入地图浏览时，始终建议仍在所嵌入的浏览旁边提供一个链接，以便用户启动全屏浏览。" 
				},
				tab5: {
					title: "发布",
					div1: "完成操作后，不要忘记通过 ArcGIS Online 中的//“共享//”按钮或应用程序项目页面与用户共享您的地图浏览。",
					div2a: "通过构建器共享浏览",
					div2b: "通过//“共享//”按钮将更新应用程序和 web 地图项目。如果您的浏览数据存储在要素服务项目中，它也会进行更新。如果通过 ArcGIS.com 地图查看器添加了其他图层，则这些图层将不会进行更新。这可能导致浏览不会按预期进行共享。具有所需 (或更多) 权限的项目将不会进行更新。",
