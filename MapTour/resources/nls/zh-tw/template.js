	 ...
	 ({
		viewer: {
			...
			errors: {
				...
				invalidConfigOwner: "嚴重錯誤: 無效的配置(需要授權的擁有者)",
				invalidConfigNoWebmap: "嚴重錯誤: 無效的配置(index.html 中未指定 web 地圖或應用程式識別碼)",
				createMap: "無法建立地圖",
				invalidApp: "嚴重錯誤: 無法載入應用程式",
				noLayer: "Web 地圖不包含用於 Map Tour 的有效數據圖層。",
				noLayerMobile: "歡迎使用 Map Tour web 應用程式。該應用程式未配置。行動裝置不支援 Map Tour 構建器。",
				noLayerView: "歡迎使用 Map Tour web 應用程式。<br />尚未配置該應用程式。",
				...
				oldBrowserExplain3: "Map Tour 構建器不適用於 Windows XP 上的 Internet Explorer 10。",
				oldBrowserClose: "關閉"
			},
			mobileHTML: {
				showIntro: "顯示標題",
			...
			builderHTML: {
				...
				dataEditionDisabled: "CSV 數據源上停用資料編輯",
				dataSourceWarning: "Map Tour 數據圖層已變更。如果圖徵 ID 不同，您必須透過<b>組織</b>重設順序和隱藏點。如果欄位名稱不同，您必須在<b>設定的數據索引標籤</b>下重設欄位設定。",
				organizeWarning: "在隱藏互動式構建器之外新增一個或多個點。",
				...
				dataPicError2: "該要求不會影響現有已發佈的 Map Tour。但是，為了使用互動式構建器，您必須先執行以下兩種操作之一解決 URL 問題:",
				dataPicError3: "編輯 URL",
				dataPicError4: "此操作將在不受支援的圖片 URL 結尾新增 <i>#isImage</i>。大多數伺服器都支援 URL 附加字尾。但在執行此操作後，應瀏覽各站點間以確認更新的圖片 URL 是否運作正常。如果各圖片均可載入，現在即可儲存 Map Tour。如果<b>圖片已損壞，則不要儲存 Map Tour</b>。請重新載入構建器並執行第二個操作。",
				dataPicError5: "限制對圖片的導覽",
				dataPicError6: "此選項導致所有 URL 都被視為影像，但您無法使用互動式構建器新增影片。如果您以後決定新增影片，可以撤消此操作。",
				...
				settingsTabLayout: "版面配置",
				settingsTabColor: "顏色",
				settingsTabLogo: "頁眉",
				settingsTabFields: "數據",
				settingsTabExtent: "範圍",
				settingsTabZoom: "縮放級別",
				settingsLayoutExplain: "選擇所需的應用程式版面配置。",
				settingsLayoutProfessional: "由三個面板構成的版面配置",
				settingsLayoutModern: "整合的版面配置",
				...
				settingsDataFieldsError: "應用程式無法確定相應的名稱、說明文字或顏色數據。請選擇要在此處使用的欄位。這些設定之後可進行變更。",
				...
				dataExplain: "該圖徵服務將新增到 web 地圖中。該服務不會與任何人共用，只有您具有新增、編輯和刪除權限。<br /><br />請注意，如果透過項目頁面共用應用程式，系統將詢問是否也要共用圖徵服務。這是必要的步驟。您仍然是唯一具有編輯權限的人。",
				dataNameLbl: "服務名稱",
				...
				introRecordBtn: "入口",
				introRecordActivate: "將第一個點做為入手點 (不在轉盤中顯示)"
			},
			addPopupJS: {
			...
			builderJS: {
				...
				shareStatus1: "未儲存導覽",
				shareStatus2: "導覽已公開共用",
				shareStatus3: "導覽已在組織內部共用",
				shareStatus4: "導覽未共用",
				...
				dataWarningExtent: "部分數據位於 web 地圖的範圍之外。這些數據不用作導覽點，如果要使用，請變更地圖範圍。",
			...
			onlinePhotoSharingCommon: {
				...
				advancedCSVLbl: "從 CSV 檔案匯入導覽數據",
				advancedCSVTip: "從 CSV 檔案匯入導覽內容。",
				advancedCommonTip: "這需要圖像和影片均已上線。",
				select: "做出選擇",
				locUse: "使用圖片地理位置",
				locExplain: "由於圖片位置可能承繼承自圖片集，造成所有照片都在相同的位置，因此您可能不想使用圖片位置。",
				locExplain2: "由於影片位置可能承繼自使用者設定，造成所有影片均位於同一位置，因此您可能不想使用影片位置。"
			},
			viewFlickr: {
				title: "Flickr 匯入",
			...
			viewYoutube: {
				title: "YouTube 匯入",
				header: "輸入 YouTube 使用者名以查詢公開共用的影片。",
			...
			viewGeoTag: {
				...
				headerExplain: "如果您的圖片具有效的地理位置，它們將自動在地圖上定位並在第二個索引標籤中列出。<br /><br />預設情況下，Picasa 和 Flickr 在您匯入圖片時不會儲存地理位置 EXIF 元數據。請勾選 Picasa/Flickr 隱私設置以啟用外部應用程式的圖片地理位置匯入與供使用。可能需要針對要使用的 EXIF 地理位置將現有圖片重新匯入到 Flickr/Picasa。<br /><br />在 Facebook 中，需要勾選每個圖片，按一下\\“編輯\\”並選擇位置。",
				leftPanelTab1: "定位",
				leftPanelTab2: "已定位",
			...
			initPopup: {
				title: "歡迎使用 Map Tour 構建器",
				prevBtn: "上一步",
				nextBtn: "下一步"
			},
			initPopupHome: {
				header1: "圖像或影片的位置在哪?",
				header2: "該助手將協助您透過從已經儲存在線上的媒體構建 Map Tour，或者將媒體匯入到您的 ArcGIS Online for Organizations 帳戶。",
				title1: "媒體已上線",
				title2: "我需要上傳我的圖像",
				hostedFSTooltip: "使用 ArcGIS Online 託管您的圖像(不支援影片)。",
				hostedFsNA: "僅適用於 ArcGIS for Organizations 發佈者和管理員使用者",
				footer1: "完成後，不要忘記透過應用程式項目頁面與觀眾共用您的 Map Tour 。",
				footer3: "下載 CSV 範本",
			...
			helpPopup: {
				...
				tab2: {
					title: "數據",
					div1: "構建 Map Tour 時要考慮的主要因素是選擇在何處儲存圖片。Map Tour 使用的圖片可以儲存在主要圖片共用服務中、任何 web 伺服器上或做為圖徵服務的附件。",
					div1a: "有關受支援的圖片格式和影片的詳細資訊，請參閱此索引標籤的最後一部分。",
					div2: "互動式構建器提供兩個選項，用於處理您的 Map Tour 中的影像:",
					div3: "<ul><li>您可以使用<b>已經在線上的照片</b>，例如儲存在 Flickr 等照片共用網站的影像或儲存在您自己網站上的影像。透過影像的 URL 將其引用到您的 Map Tour 中。</li><li>還可以<b>透過電腦將照片</b>直接上傳到您的 Map Tour。該上傳選項需要您具有 ArcGIS for Organizations 帳戶，並具有發佈者或管理員權限，因為系統將自動為您建立託管的圖徵服務，您可將照片做為附件儲存在其中。</li></ul>",
					div4: "主要用例包括:",
					div4b: "<b>您的照片尚未託管</b>，且您擁有 ArcGIS for Organizations 帳戶: 使用託管的圖徵服務是您的最佳選擇。與公開照片共用服務一樣，我們將最佳化您的圖片以建立快速載入影像，您將可以存取 ArcGIS 平臺的所有管理和數據管理功能。",
					div5: "<b>您不是組織的成員</b>: 首先，您需要將圖片上傳到照片共用網站或您自己的 web 伺服器上。構建器將協助您使用這些繼續託管在其原始位置的圖片。",
					div6: "<b>您希望重複使用現有圖徵服務</b>將圖片儲存為附件或引用外部圖片: 請參閱下面的詳細描述部分。",
					div7: "<b>您是上一版 Map Tour </b>範本的使用者，且已經擁有引用圖片和縮圖的 CSV: 您可以匯入該 CSV 並最佳化數據。構建器僅支援使用緯度/經度欄位的 CSV; 以地址為基礎的 CSV 可以繼續在您的 web 地圖中使用 (請參閱以下部分)。",
					div8: "從線上照片共用服務中匯入",
					...
					div11: "如果選擇自己託管圖片，則需要手動建立圖片的縮圖。使用全解析度的圖片建立縮圖將導致效能不佳。因此，強烈建議您使用線上照片共用服務或圖徵服務來進行此操作。",
					div12: "使用現有圖徵服務或 Shapefile",
					div13: "任何點圖徵服務或 shapefile 都可做為 Map Tour 數據源。只需透過 arcgis.com 地圖檢視器新增為 web 地圖的圖層。如果應用程式在您的圖層中找到預定的屬性，則所有構建器功能都將可用。",
					...
					div154: "縮圖",
					div155: "顏色 ",
					div16: "如果在使用圖徵服務時，應用程式未找到匹配欄位，在您透過構建器配置要使用的欄位之前，檢視器無法使用。新增到 web 地圖的 CSV 和 shapefile 圖層需要包含所有必填欄位，否則構建器將無法運作。",
					div162: "使用將圖片儲存為附件的圖徵服務時，<b>只能使用帶有兩個附件的圖徵</b>。第一個附件定義主要圖片，第二個附件定義縮圖。",
					div17: "對於不帶附件的圖徵服務，圖片和縮圖欄位為必填欄位；對於帶附件的圖徵服務，圖片和縮圖欄位為可選欄位 (但強烈建議填寫)。如果您的服務已啟用附件，構建器將讓您以附件形式上傳圖片。如果沒有啟用，您只能編輯圖片和縮圖 URL。",
					...
					div23: "使用帶有附件的圖徵服務時，互動式構建器不提供可包含影片的對話方塊，但可以透過在互動式構建器外部編輯數據進行。在 arcgis.com 地圖檢視器中，如果將圖片欄位修改為指向外部影片，並在 URL 尾部新增特殊參數 (#isVideo)，則您的媒體將被視為影片。",
					div24: "請注意，您仍需要兩個有效的圖片附件，否則將不會使用此點。使用不含圖片和縮圖欄位的圖徵服務附件時，無法使用影片。"
				},
				tab3: {
					title: "自訂",
					div1: "構建器提供幾個自訂選項，可以透過頂部面板中的\\“設定\\”按鈕進行存取。如果沒有找到所需的選項，我們還提供一個可下載版本，您可以在 web 伺服器上進行部署，並可強化以符合您的需求。",
					div2: "建議您使用託管的版本，除非:",
					div3: "<li>它不提供您想要的 UI 自訂，例如使用標題背景圖像。</li><li>您是開發者，並且希望增強應用程式。</li>",
					div4: "可透過 web 地圖或 web 製圖應用程式識別碼配置可下載版本。主要用例有:",
					div41: "使用託管環境中的互動式構建器來構建 Map Tour，並透過 web 製圖應用程式識別碼來配置範本。將套用透過互動式構建器定義的設定。",
					div42: "在互動式構建器外部構建您的 web 地圖，並透過 web 地圖識別碼配置範本。您需要閱讀說明文件以瞭解如何配置範本。",
					div43: "請注意，可下載的版本中提供互動式構建器，但對於 Internet Explorer 10 之前的舊版瀏覽器仍有技術限制。",
					div5: "要下載最新版本的範本並瞭解其使用方法，請存取 <a href='https://github.com/Esri/map-tour-storytelling-template-js' target='_blank'>GitHub 項目頁面</a>。"
				},
				tab4: {
					title: "提示",
					div0: "支援的瀏覽器",
					div0a: "IE8+ 支援 Map Tour 檢視器。IE9+ 支援互動式構建器。我們已經在所有主要瀏覽器中進行測試，但如果您遇到了困難，則建議您使用 Chrome。",
					div0b: "如果您遇到任何困難，請與我們聯繫。此外，使用 CSV 範本構建 Map Tour，可將您與構建器介面的互動降至最低。",
					div1: "圖片",
					...
					div10: "嵌入模式",
					div11: "如果您想要透過 iframe 將範本嵌入另一個網站，在 URL 末端添加選項參數 \"&embed\" 將移除標頭。此模式也可以透過配置參數於可下載的版本中設定。",
					div12: "避免使用狹窄的 iFrame 寬度，因其可能導致 Map Tour 切換至回應式觸控小螢幕的版面配置。為了擴大使用簡易性，在嵌入 Map Tour 時，通常建議在所嵌入的導覽旁邊提供一個連結，以便使用者啟動全螢幕導覽。" 
				},
				tab5: {
					title: "發佈",
					div1: "完成操作後，不要忘記透過 ArcGIS Online 中的\\“共用\\”按鈕或應用程式項目頁面與觀眾共用您的 Map Tour 。",
					div2a: "透過構建器共用導覽",
					div2b: "透過\\“共用\\”按鈕可更新應用程式和 web 地圖項目。如果您導瀏覽數據儲存在圖徵服務項目中，也會更新。如果透過 ArcGIS.com 地圖檢視器新增其他圖層，則這些圖層將不會進行更新。這可能導致導覽不會按預期進行共用。具有必要 (或更多) 權限的項目將不會更新。",
					div2c: "透過 ArcGIS Online 共用導覽",
					div2d: "透過 ArcGIS Online 共用應用程式時，系統將在必要時要求您更新非以相同方式共用的所有相關資源(web 地圖、圖徵服務和裝飾圖層)。如果 Map Tour 處於公開狀態，且其中一個資源並沒有共用給觀眾，則使用者將被重新導向至 ArcGIS Online 登錄頁面。",
					div3t: "圖徵服務安全性",
					div3a: "如果使用透過 Map Tour 構建器建立的託管圖徵服務，應用程式將為您管理服務安全性，即使公開共用服務，您也是唯一具有編輯權限的人員。",
					div3t2: "在與觀眾共用導覽之前",
					div3: "請確保在未使用 ArcGIS.com 帳戶登錄時，導覽也可正常運作。要使用的 URL 不應重新導向至登錄頁面或構建器模式。",
			...
			share: {
				...
				firstSaveQ2: "我的導覽是否已共用?",
				firstSaveA2: "您的導覽目前並未共用。若要共用，請使用\\“共用\\”按鈕。",
				shareTitle: "共用您的導覽",
				sharePrivateHeader: "您的導覽尚未共用，是否要共用?",
				...
				shareLinkHeader: "與觀眾共用導覽",
				shareLinkOpen: "打開",
				shareQ1Opt1: "如何將導覽設置為私有?",
				shareQ1Opt2: "如何將導覽設置為私有或將其公開共用？",
				shareA1: "請使用<a href='%LINK1%' target='_blank'>應用程式項目頁面</a>上的 %SHAREIMG%。如果還想要取消共用 web 地圖，請使用 <a href='%LINK2%' target='_blank'>web 地圖項目頁面</a>。",
				...
				shareQ3: "數據儲存在哪兒?",
				shareA3: "瀏覽配置儲存在<a href='%LINK1%' target='_blank'>此 web 地圖項目</a>和<a href='%LINK2%' target='_blank'>此 web 應用程式項目</a>中。Flickr、Picasa 和 Facebook 圖像以及 YouTube 影片仍可在其網站引用，並且不會複製到 ArcGIS Online。",
