define(["dojo/Evented", "dojo/_base/declare", "dojo/_base/window",
  ...
  "dojo/query", "dijit/focus", "dojo/keys", "dojo/Deferred"
], function(
  Evented, declare, win, fx, html, lang, has, dom, domClass, domStyle,
  domAttr, domConstruct, domGeometry, on, mouse, query, focus, keys,
  Deferred) {
  return declare([Evented], {
      ...
      on(this.pTools, "keydown", lang.hitch(this, function(event) {
        ...
        if (title) {
          ...
          if (event.keyCode === keys.LEFT_ARROW || event.keyCode ===
            ...
            (event.keyCode === keys.LEFT_ARROW) ? this._showPreviousPage(
              title) : this._showNextPage(title);
          }
        }
      }));
    ...
    createTool: function(tool, panelClass) {
      var name = tool.name;
      var pressed = (this.config.activeTool === name) ? true :
        false;
      // add tool
      ...
      domConstruct.create("div", {
        ...
        title: name,
        role: "presentation",
        innerHTML: "<img class='pageIcon' src ='images/icons_" +
          this.config.icons + "/" + name + ".png' alt=" +
          name +
          "/>"
      }, pageHeader);
