...
"esri/tasks/geometry"
    ...
    var App =  declare("utilities.App",null,{
        ...
        init: function(){
            ...
            //need to set the sharing url here so that when we query the application and organization the correct 
            //location is searched. 
            this.setDefaults();
            var orgDef = this.queryOrganization();
            orgDef.then(dojo.hitch(this, function(){
              var localizationDef = this.getlocalization();
              localizationDef.then(dojo.hitch(this,function(){
                  this.queryApplication().then(dojo.hitch(this,function(results){
        ...
        //itemData.values then the app was configurable so overwrite the default values with the configured values. 
            ...
            if(this.config.appid){
               return utils.getItem(this.config.appid).then(lang.hitch(this,function(response){
                    ...
                    if(this.config.tabs){
                      var tabs = this.config.tabs.split(",");
                      
                      this.config.navigationTabs = dojo.map(tabs, function(tab){
                        return {
                          "title": tab
                        }
                      });
