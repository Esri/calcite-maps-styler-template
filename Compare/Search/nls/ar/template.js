define(
({
  viewer:{
    title: "مقارنة خرائط الويب",
    main:{
        loading:{
            loadingMessage: "جارِ تحميل خرائط الويب...",
            title: "مقارنة خرائط الويب"
        },
        contextMenu:{
            title: "انقر بزر الفأرة الأيمن على الخريطة لمزيد من الخيارات.",
            zoom: "تكبير إلى الحد الأولي",
            scale: "مقياس رسم",
            location: "الموقع",
            scaleAndLocation: "مقياس الرسم والموقع",
            adjust: "تعديل خرائط أخرى إلى هذا"
        },
      title: "مقارنة خرائط الويب",
      scaleBarUnits: "metric" //"english (for miles) or "metric" (for km) - don't translate.
    },
    errors:{
      createMap: "يتعذر إنشاء الخريطة",
      general:"Error",
      webmapDescription: "...لا يوجد وصف…",
      legend:"No legend",
      bingMessage: "مفتاح Bing مفقود"
    },
    footer:{
    	label:"A map from:"
    },
    sidePanel:{
        save:{
            label: "حفظ خرائط الويب",
            alt: "حفظ",
            title: "حفظ خرائط الويب",
            message: "يتم حفظ خرائط الويب حاليًا",
            changeMessage: "تم تغيير خرائط الويب",
            clearMessage: "تم مسح خرائط الويب"
        },
        search:{
            alt: "بحث",
            title: "بحث",
            placeholder: "إدخال نص البحث",
            message: "جارِ البحث...",
            results: "تم العثور على خرائط ويب"
        },
        clear:{
            alt: "مسح",
            title: "مسح خرائط الويب (استخدم خرائط الويب الافتراضية)"
        },
        find:{
            label: "العثور على خرائط الويب"
        },
        sync:{
            label: "تزامن الخرائط:",
            scaleLabel: "مقياس رسم",
            locationLabel: "موقع"
            
        },
        map:{
            label:"MAP INFORMATION",
            descriptionLabel: "الوصف",
            contentLabel: "المحتوى",
            legendLabel: "مفتاح الخريطة"
        }
    }
  }
})

);