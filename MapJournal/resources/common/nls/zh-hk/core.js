	 ...
	 ({
		commonCore: {
			...
			share: {
				...
				firstSaveHeader: "應用程式現已儲存在 %PORTAL% 中，但尚未進行共用。",
				firstSavePreview: "預覽",
				firstSaveShare: "共用",
				firstSaveA1: "如果不熟悉 %PORTAL% 或需要使用捷徑存取構建器介面，則可以儲存以下連結：%LINK1%",
				firstSaveA1bis: "也可在 <a href='%LINK2%' target='_blank'>%PORTAL% 內容文件夾</a>中找到應用程式。",
				shareTitle: "共用應用程式",
				sharePrivateHeader: "應用程式尚未共用，是否要進行共用？",
				sharePrivateBtn1: "公開共用",
				sharePrivateBtn2: "與我的組織共用",
				sharePrivateWarning: "已禁止共用 %WITH%，因為您不是 <a href='%LINK%' target='_blank'>webmap</a> 的擁有者。",
				sharePrivateWarningWith1: "公開",
				sharePrivateWarningWith2: "公開並在組織內共用",
				...
				shareQ1Opt2: "如何將應用程式私用或公開共用?",
				shareA1: "在<a href='%LINK1%' target='_blank'>應用程式項目頁面</a>上使用 %SHAREIMG%。",
				shareQ2bis: "如何返回構建器介面？",
				shareA2div1: "儲存並重新使用以下連結 %LINK1%，或使用<a href='%LINK2%' target='_blank'>應用程式項目頁面</a>。",
				shareA2div2: "做為應用程式擁有者，登錄到 %PORTAL% 後，應用程式將包括用於打開構建器的按鈕：",				
				shareQ3: "數據儲存在哪兒?",
				shareA3: "%TPL_NAME% 數據和配置均儲存在<a href='%LINK2%' target='_blank'>此 web 應用程式項目</a>中。如果已使用 Flickr、Picasa、Facebook 或 YouTube 進行匯入，則圖像和影片不會複製到 %PORTAL% 中。"
			},
			settings: {
				header: "設定",
